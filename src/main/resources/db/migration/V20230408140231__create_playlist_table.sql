...
(
    ...
    `name` VARCHAR(64) NOT NULL COMMENT '歌单名',
    `description` TEXT NULL COMMENT '歌单简介',
    `cover_id` VARCHAR(32) NULL COMMENT '歌单封面ID',
    `status` VARCHAR(32) DEFAULT 'DRAFT' NOT NULL COMMENT '歌单状态，DRAFT-草稿，PUBLISHED-已上架，CLOSED-已下架',
    `creator_id` VARCHAR(32) NOT NULL COMMENT '创建者ID',
    `created_time` datetime(6) NOT NULL COMMENT '创建时间',
    `updated_time` datetime(6) NOT NULL COMMENT '更新时间',
    CONSTRAINT `c_playlist_file_id`
        FOREIGN KEY (`cover_id`) REFERENCES `file` (`id`),
    CONSTRAINT `c_playlist_creator_id`
